...
to its own output directory and plots against the Tutorial 2 no-intervention
baseline so you can see the impact of each intervention.
...
# Toggle these to run with each intervention separately and compare results.
# Both True (default) saves to tutorial_3_results.
# One True saves to tutorial_3_results_ts or tutorial_3_results_itn.
# Both False saves to tutorial_3_results_no_interventions (same as Tutorial 2 baseline).
...
def get_run_suffix():
    """
    Return a filename suffix based on which interventions are active.
    Used to keep output directories separate across runs.
    """
...
def plot_results(output_path):
    ...
    If tutorial_2_results is present, its InsetChart is used as the reference
    (plotted in red) so the intervention impact is visible directly: treatment
    seeking reduces the infected fraction, and ITNs reduce the daily biting rate
    ...
    # Use Tutorial 2's InsetChart as the no-intervention baseline if available.
    reference = None
